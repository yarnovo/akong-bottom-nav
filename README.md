...
akong BottomNav · 极简 · 跨端 (Web + React Native)
...
<BottomNav variant="primary" size="md" onClick={...}>下单</BottomNav>
<BottomNav variant="secondary" loading>处理中</BottomNav>
<BottomNav variant="ghost" iconLeft={<Plus />}>新建</BottomNav>
<BottomNav variant="destructive" disabled>删除</BottomNav>
<BottomNav variant="link">查看详情</BottomNav>
```
...
<BottomNav variant="primary" size="md" onPress={...}>下单</BottomNav>
```
...
- **两端实现**：`BottomNav.tsx` (Web · DOM `<button>`) + `BottomNav.native.tsx` (RN · `<Pressable>`)
